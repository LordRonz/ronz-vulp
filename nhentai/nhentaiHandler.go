...
func NhentaiRandom(bot *linebot.Client, event *linebot.Event) {
	...
	NhentaiSearchById(bot, event, splittedUrl[len(splittedUrl) - 2])
}
...
func NhentaiSearchById(bot *linebot.Client, event *linebot.Event, id string) {
	client := &http.Client{}

	randomUrl := fmt.Sprintf("https://nhentai.net/api/gallery/%s", id)
	...
	if nhentaiRes.Error == "does not exist" {
		if _, err = bot.ReplyMessage(event.ReplyToken, linebot.NewTextMessage("No result for " + id)).Do(); err != nil {
			log.Print(err)
		}
		return
	}
